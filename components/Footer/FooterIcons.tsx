  ...
  return (
    <div className="flex h-full">
      <div className={`${!copySuccess && shouldApplyWidthClass ? 'w-[15.8rem]' : ''}`}>
        {copySuccess && (
          <p className={`${shouldApplyWidthClass ? 'flex-grow flex dark:text-blendedCyan w-[17.5rem] h-full text-green-500 border bg-white/80 dark:bg-black/80 rounded-t-md border-primary px-6 pb-[2px] pt-1 text-xs leading-normal transition duration-150 ease-in-out hover:bg-opacity-10 focus:outline-none border-green-500 dark:border-blendedCyan focus:ring-0 dark:hover:bg-opacity-10' : 'flex-grow dark:text-blendedCyan w-[8rem] rounded-t-md h-full text-green-500 border bg-white/80 dark:bg-black/80 border-primary px-3 pb-[2px] pt-1 text-xs leading-normal transition duration-150 ease-in-out hover:bg-opacity-10 focus:outline-none border-green-500 dark:border-blendedCyan focus:ring-0 dark:hover:bg-opacity-10'}`}>
            <span className={`${shouldApplyWidthClass ? 'text-black dark:text-white text-xs' : 'hidden'}`}>Send me a DM😎</span>
            <span className="ml-1 flex text-xs light:text-black">{copiedValue} Copied<RiFileCopyFill className={`${shouldApplyWidthClass ? 'ml-1 visible w-4 h-4 text-xs' : 'hidden'}`} /></span>
          </p>
        )}
      </div>
      <button
