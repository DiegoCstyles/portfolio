...
const Navbar = ({ currentLanguage, onLanguageChange }: NavbarProps) => {
  ...
  useEffect(() => {
    ...
    return (
      <nav className={`nav ${isNavbarVisible ? 'fixed top-0 w-full z-40 dark:bg-black/90 bg-white/90' : 'fixed top-0 w-full z-40'}`}>
        <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-0.5">
          ...
          >
            {isSmallScreen ? (
              <div className={isNavbarVisible ? 'bg-black/90' : ''}>
